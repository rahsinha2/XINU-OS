...
static void smc_phy_configure(struct ether *dev);
...
/*
 ...
 .
*/
static void smc_reset (struct ether *dev)
{
	PRINTK2 ("%s: smc_reset\n", SMC_DEV_NAME);
...
/*
 ...
 .	3.  Enable interrupts
*/
static void smc_enable(struct ether *dev)
{
	...
	/* enable interrupt*/
  SMC_SELECT_BANK( dev, 2 );
	SMC_outb( dev, SMC_INTERRUPT_MASK, IM_REG);
...
*/
static void smc_halt(struct ether *dev)
{
	PRINTK2("%s: smc_halt\n", SMC_DEV_NAME);
...
int smc_send(struct ether *dev, void *packet, int packet_length)
{
  ...
  // Print intterupt Mask
  SMC_SELECT_BANK (dev, 2);
  kprintf("\n MASK IM_RCV_INT : %d", ((SMC_inw (dev, IM_REG) & IM_RCV_INT))?1:0);
  kprintf("\n MASK IM_ALLOC_INT : %d", ((SMC_inw (dev, IM_REG) & IM_ALLOC_INT))?1:0);
  kprintf("\n MASK IM_TX_EMPTY_INT : %d", ((SMC_inw (dev, IM_REG) & IM_TX_EMPTY_INT))?1:0);
  kprintf("\n MASK IM_TX_INT : %d", ((SMC_inw (dev, IM_REG) & IM_TX_INT))?1:0);
    
  kprintf("\n INT IM_RCV_INT : %d", ((SMC_inw (dev, SMC91111_INT_REG) & IM_RCV_INT))?1:0);
  kprintf("\n INT IM_ALLOC_INT : %d", ((SMC_inw (dev, SMC91111_INT_REG) & IM_ALLOC_INT))?1:0);
  kprintf("\n INT IM_TX_EMPTY_INT : %d", ((SMC_inw (dev, SMC91111_INT_REG) & IM_TX_EMPTY_INT))?1:0);
  kprintf("\n INT IM_TX_INT : %d\n", ((SMC_inw (dev, SMC91111_INT_REG) & IM_TX_INT))?1:0);
	...
	if (numPages > 7) {
		kprintf ("%s: Far too big packet error. \n", SMC_DEV_NAME);
		return 0;
	}
...
again:
	...
	while ( saved_ptr & PTR_NOTEMPTY )
		kprintf ("Write data fifo not empty!\n");
  ...
  if (poll4int(dev, IM_RCV_INT, SMC_TX_TIMEOUT)) {
      PRINTK2 ("%s: Packet not recieved  \n", SMC_DEV_NAME);  
  }else{
      PRINTK2 ("%s: Packet recieved  \n", SMC_DEV_NAME);
  }
...
static int smc_write_hwaddr(struct ether *dev)
{
	int i;
	...
	for (i = 0; i < 6; i += 2) {
		...
		address = dev->devAddress[i + 1] << 8;
		address |= dev->devAddress[i];
		SMC_outw(dev, address, (ADDR0_REG + i));
	}
#else
	for (i = 0; i < 6; i++)
		SMC_outb(dev, dev->devAddress[i], (ADDR0_REG + i));
#endif
	swap_to(FLASH);
	return 0;
}
...
int smc_read_hwaddr(struct ether *dev, uchar *macaddr)
{
	...
	SMC_SELECT_BANK(dev, 1);
  for (i = 0; i < 6; ++i){
      macaddr[i] = SMC_inb(dev, (ADDR0_REG + i));
  }
  return 1;
#else
  return 0;
 ...
 int smc_init(struct ether *dev)
{
  ...
  /* Go this thing from Linux driver*/
	smc_enable (dev);
...
/*------------------------------------------------------------
 ...
 .-------------------------------------------------------------*/
static int smc_get_regbit(struct ether *dev, int bank, int ioaddr, int reg, unsigned int bit)
{
	SMC_SELECT_BANK( dev, bank );
...
/*------------------------------------------------------------
 ...
 .-------------------------------------------------------------*/
static void smc_modify_reg(struct ether *dev, int bank, int ioaddr, int reg, word val)
{
	SMC_SELECT_BANK( dev, bank );
...
/*------------------------------------------------------------
 ...
 .-------------------------------------------------------------*/
static int smc_get_reg(struct ether *dev, int bank, int ioaddr, int reg)
{
	SMC_SELECT_BANK( dev, bank );
...
#if (SMC_DEBUG > 2 )
	kprintf ("smc_write_phy_register(): phyaddr=%x,phyreg=%x,phydata=%x\n",
		phyaddr, phyreg, phydata);
	smc_dump_mii_stream (bits, sizeof bits);
#endif
}
