...
again:
 ...
 /* enable the interrupts */ 
	SMC_ENABLE_INT(dev, (IM_TX_INT | IM_TX_EMPTY_INT));
 ...
 /* restore previously saved registers */
	...
	/* As send function add control bytes to original packet, packet 
	 * length is increasing which is causing problem in etherwrite 
	 * function so returning original packet length 
	 */
...
int smc_rcv(struct ether *dev, void *buf)
{
	...
	if ( !(status & RS_ERRORS ) ){
		...
		/* QUESTION:  Like in the TX routine, do I want
		   ...
		   performance	*/
		SMC_insl( dev, SMC91111_DATA_REG, buf,
			packet_length >> 2 );
		...
		SMC_insw(dev, SMC91111_DATA_REG , buf,
			packet_length >> 1);
		...
		print_packet( buf, packet_length );
#endif
	} else {
		/* error ... */
		/* TODO ? */
...
/*------------------------------------------------------------
 ...
 .-------------------------------------------------------------*/
#ifndef CONFIG_SMC91111_EXT_PHY
static word smc_read_phy_register (struct ether *dev, byte phyreg)
{
	int oldBank;
	int i;
	byte mask;
...
/*------------------------------------------------------------
 ...
 .-------------------------------------------------------------*/
static void smc_write_phy_register (struct ether *dev, byte phyreg,
	word phydata)
{
	int oldBank;
	int i;
